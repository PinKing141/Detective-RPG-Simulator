...
def _restore_investigation_snapshot(
    ...
    board = DeductionBoard(hypothesis=restored_hypothesis)
    board.sync_from_state(loaded_state)
    note = f"Loaded saved investigation for {case_id}."
    if restore_note:
        note = f"{note} {restore_note}"
    return loaded_state, loaded_presentation, board, note
...
def _choose_suspect_candidate(truth, presentation, known_ids: list) -> tuple[str, object] | None:
    candidate_ids = suspect_candidate_ids(truth, presentation, known_ids)
    people = [truth.people[person_id] for person_id in candidate_ids if person_id in truth.people]
    if not people:
        return None
    if len(people) == 1:
        return people[0].name, people[0]
...
def _hypothesis_summary_lines(board: DeductionBoard, truth, presentation) -> list[str]:
    ...
    if board.hypothesis.reasoning_steps:
        lines.append("Reasoning chain:")
        for step in board.hypothesis.reasoning_steps:
            summary = _selected_evidence_summary(presentation, step.evidence_id) or "missing evidence"
            lines.append(f"- {_format_claim(step.claim)} <- {summary}")
...
def _choose_reasoning_steps(
    presentation,
    claims: list[ClaimType],
    evidence_ids: list,
) -> list[ReasoningStep]:
    items = [item for item in presentation.evidence if item.id in set(evidence_ids)]
    steps: list[ReasoningStep] = []
    for claim in claims:
        print(f"Link evidence to claim: {_format_claim(claim)}")
        for idx, item in enumerate(items, start=1):
            print(f"{idx}) {_format_evidence_summary(item)}")
        choice = input("> ").strip()
        if not choice.isdigit():
            return []
        index = int(choice) - 1
        ...
        steps.append(
            ReasoningStep(
                ...
                note=describe_reasoning_step(presentation, selected.id, claim),
            )
        )
    return steps
...
def _choose_evidence(
    ...
    items.sort(key=lambda item: (0 if item.id in recommended_set else 1, presentation.evidence.index(item)))
    if not items:
    ...
    for idx, item in enumerate(items, start=1):
        if item.id in recommended_set:
            print("   Recommended for the current theory.")
...
def _run_smoke(seed: int, case_id: str | None, case_archetype: CaseArchetype | None) -> None:
    ...
    witnesses = [p for p in truth.people.values() if RoleTag.WITNESS in p.role_tags]
    if witnesses:
    ...
    candidate_evidence_ids = list(board.known_evidence_ids)
    suspect = None
    claims: list[ClaimType] = []
    evidence_ids: list = []
    reasoning_steps = []
    best_validation = None
    tier_rank = {"failed": 0, "shaky": 1, "clean": 2}
    for suspect_id in suspect_candidate_ids(truth, presentation, candidate_evidence_ids):
        trial_claims = _supported_claims(presentation, candidate_evidence_ids, suspect_id)[:3]
        if not trial_claims:
            continue
        trial_evidence_ids = recommended_hypothesis_evidence_ids(
            presentation,
        ...
        if not trial_evidence_ids:
            continue
        trial_steps = auto_build_reasoning_steps(
            presentation,
            trial_evidence_ids,
            suspect_id,
            trial_claims,
        ...
        trial_board = DeductionBoard(
            hypothesis=Hypothesis(
                suspect_id=suspect_id,
                claims=trial_claims,
                evidence_ids=trial_evidence_ids,
                reasoning_steps=trial_steps,
            )
        )
        validation = validate_hypothesis(truth, trial_board, presentation, state)
        if best_validation is None or (
            (validation.probable_cause, validation.is_correct_suspect, tier_rank.get(validation.tier.value, -1)) >
            (best_validation.probable_cause, best_validation.is_correct_suspect, tier_rank.get(best_validation.tier.value, -1))
        ):
            ...
            reasoning_steps = trial_steps
    if suspect is None:
        print("[smoke] No suspect hypothesis is supportable from collected evidence; aborting.")
        return
    if not evidence_ids:
    ...
    print(f"[smoke] Validation: {validation.summary}")
    for line in validation.supports:
        print(f"[smoke] support: {line}")
    for line in validation.missing:
        print(f"[smoke] gap: {line}")
    ...
    print(f"[smoke] Arrest: {arrest_result.summary}")
    outcome = resolve_case_outcome(validation)
    print(f"[smoke] Outcome: {outcome.arrest_result.value}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Fallback CLI loop.")
    ...
    while True:
        ...
        if choice == "1":
            ...
            board.sync_from_state(state)
            selection = _choose_suspect_candidate(
                truth,
                presentation,
                board.known_evidence_ids,
            )
            if not selection:
            ...
            reasoning_steps = _choose_reasoning_steps(presentation, claims, evidence_ids)
            if len(reasoning_steps) < len(claims):
                print("Invalid reasoning chain selection.")
                continue
            result = set_hypothesis(
                state,
                board,
        ...
        if result.notes:
            for note in result.notes:
                print(f"- {note}")
        maybe_print_investigation_guidance(
            result,
            truth,
            presentation,
            state,
