...
def _run_path(seed: int, path_name: str) -> None:
    ...
    if path_name == "witness_only":
        ...
        witnesses = [person for person in truth.people.values() if RoleTag.WITNESS in person.role_tags]
        if witnesses:
            interview(truth, presentation, state, witnesses[0].id, location_id)
        request_cctv(truth, presentation, state, location_id)
        submit_forensics(truth, presentation, state, location_id, item_id=item_id)
        for person in witnesses[1:]:
            interview(truth, presentation, state, person.id, location_id)
        evidence = [
            item
    ...
    candidate_ids = [item.id for item in evidence if item.id in state.knowledge.known_evidence]
    ...
    set_hypothesis(state, board, suspect.id, claims, evidence_ids, reasoning_steps)
    arrest(
        truth,
        presentation,
        state,
